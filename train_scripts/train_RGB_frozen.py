...
from models.with_mobilenet_frozen import PoseEstimationWithMobileNet
from modules.loss import l2_loss
from modules.load_state import load_state, load_state_rgbd,load_from_mobilenet
...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, num_img_input,base_lr, batch_size, batches_per_iter,
    ...
    net = PoseEstimationWithMobileNet(num_img_input,num_refinement_stages)
    ...
    dataset = CocoTrainDataset(prepared_train_labels, train_images_folder,
                               ...
                               transform=transforms.Compose([
                                   ...
                                   CropPad(pad=(128, 128,128),img_input = num_img_input),
                                   Flip()]))
    ...
    if checkpoint_path:
        ...
        if from_mobilenet:
            ...
            elif num_img_input == 4:
                load_state_rgbd(net, checkpoint)
            if not weights_only:
    ...
    for epochId in range(current_epoch, 300):
        ...
        for batch_data in train_loader:
            ...
            if num_iter % log_after == 0:
                print('Iter: {}'.format(num_iter),'epochId: {}'.format(epochId),'lr:{}'.format(scheduler.get_lr()[0]))
                for loss_idx in range(len(total_losses) // 2):
            ...
            if num_iter % val_after == 0:
                print('Validation...')
                evaluate(val_labels, val_output_name, val_images_folder, net,img_input = num_img_input)
                net.train()
...
if __name__ == '__main__':
    ...
    train(args.prepared_train_labels, args.train_images_folder, args.num_refinement_stages, args.num_img_input,args.base_lr, args.batch_size,
