...
from modules.loss import l2_loss, l2_loss_3DPAF
from modules.load_state import load_state, load_from_mobilenet, load_from_light
...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
          num_workers, checkpoint_path, weights_only, from_light, checkpoints_folder, log_after,
    ...
    #net = net.cuda()
    net.train()
    summaryWriter = SummaryWriter("./runs/V2_exp2")
        ...
        for batch_data in train_loader:
            ...
            if num_iter % log_after == 0:
                ...
                for loss_idx in range(len(total_losses) // 2):
                    ...
                    summaryWriter.add_scalars('training_loss_pafs', {'stage{}'.format(loss_idx + 1): total_losses[loss_idx * 2 + 1] / log_after}, num_iter)
                    summaryWriter.add_scalars('training_loss_heatmaps', {'stage{}'.format(loss_idx + 1): total_losses[loss_idx * 2] / log_after}, num_iter)                                  
                for loss_idx in range(len(total_losses)):
                    total_losses[loss_idx] = 0
            if num_iter % (checkpoint_after / 5) == 0:
...
if __name__ == '__main__':
    ...
    args.experiment_name = "rgb_rgbd_V2_loss"
    ...
    args.checkpoint_path = "./rgb_rgbd_V2_checkpoints/checkpoint_iter_15000.pth"
    weights_only = False
    from_light = False
    print("Training...")
    train(args.prepared_train_labels, args.train_images_folder, args.num_refinement_stages, args.base_lr, args.batch_size,
          args.batches_per_iter, args.num_workers, args.checkpoint_path, weights_only, from_light,
