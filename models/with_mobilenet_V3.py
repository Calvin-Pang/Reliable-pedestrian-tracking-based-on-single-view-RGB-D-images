...
class InitialStage(nn.Module):
    def __init__(self, num_channels, num_heatmaps, num_pafs):
        super().__init__()
        self.trunk_kpt = nn.Sequential(
            conv(num_channels, num_channels, bn=False),
            conv(num_channels, num_channels, bn=False),
            conv(num_channels, num_channels, bn=False)
        )
        for p in self.trunk_kpt.parameters():
            p.requires_grad = False
        print('intial trunk_kpt frozen')
        self.trunk_paf = nn.Sequential(
            conv(num_channels, num_channels, bn=False),
            conv(num_channels, num_channels, bn=False),
            conv(num_channels, num_channels, bn=False)
        )
        ...
        for p in self.heatmaps.parameters():
            p.requires_grad = False
        print('intial trunk_kpt frozen')
    ...
    def forward(self, x):
        trunk_kpf_features = self.trunk_kpt(x)
        heatmaps = self.heatmaps(trunk_kpf_features)
        trunk_paf_features = self.trunk_paf(x)
        pafs = self.pafs(trunk_paf_features)
...
class RefinementStage(nn.Module):
    def __init__(self, in_channels, out_channels, num_heatmaps, num_pafs):
        ...
        for p in self.trunk_kpt.parameters():
            p.requires_grad = False
        print('refinement trunk_kpt frozen')
        self.trunk_paf = nn.Sequential(
            RefinementStageBlock(in_channels, out_channels),
            RefinementStageBlock(out_channels, out_channels),
            RefinementStageBlock(out_channels, out_channels),
    ...
    def forward(self, x):
        trunk_kpt_features = self.trunk_kpt(x)
        heatmaps = self.heatmaps(trunk_kpt_features)
        trunk_paf_features = self.trunk_paf(x)
        pafs = self.pafs(trunk_paf_features)
        return [heatmaps, pafs]
...
class PoseEstimationWithMobileNet(nn.Module):
    ...
    def forward(self, x):     
        rgb, rgbd = x.split([3, 1], dim=1) 
        rgbd = rgbd.repeat(1,3,1,1)
