...
from datasets.coco import CocoDemoDataset
...
def evaluate(file_names, output_name, images_folder, net, img_input = 3,multiscale=False, visualize=False, img_save_folder='/content/drive/MyDrive/'):
    ...
    dataset = CocoDemoDataset(file_names, images_folder)
    img_list = dataset._file_names
    coco_result = []
    for idx,sample in enumerate(dataset):
        ...
        # image_idx = idx
        if img_input == 3:
            avg_heatmaps, avg_pafs = infer(net, img, scales, base_height, stride, img_mean=(128, 128, 128))
        elif img_input == 4:
            avg_heatmaps, avg_pafs = infer(net, img, scales, base_height, stride, img_mean=(128, 128, 128,128))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file-names', type=str, required=True, help='path to json with keypoints val labels')
    ...
    evaluate(args.file_names, args.output_name, args.images_folder, net,args.num_img_input, args.multiscale, args.visualize,args.img_save_folder)
    print('Demo on dataset completed!')