    ...
    def draw(self, img):
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                # print(x_a,y_a)
                cv2.putText(img, str(kpt_a_id), (int(x_a), int(y_a)),cv2.FONT_ITALIC,0.4,(int(color[0]), int(color[1]), int(color[2])),1)
                cv2.circle(img, (int(x_a), int(y_a)), 2, (int(set_color[0]), int(set_color[1]), int(set_color[2])), -1)
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            global_kpt_b_id = self.keypoints[kpt_b_id, 0]
            if global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                # print(x_b,y_b)
                cv2.circle(img, (int(x_b), int(y_b)), 2, (int(set_color[0]), int(set_color[1]), int(set_color[2])), -1)
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), (int(color[0]), int(color[1]), int(color[2])),1,lineType=cv2.LINE_AA)
...
def track_poses(previous_poses, current_poses, threshold=5, smooth=False):
