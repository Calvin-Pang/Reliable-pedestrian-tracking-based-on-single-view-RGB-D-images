...
class CocoTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(os.path.join(self._images_folder, label['img_paths']), -1)
        # print(image.shape)
...
class CocoValDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.imread(os.path.join(self._images_folder, file_name), -1)
        img_show = cv2.imread(os.path.join(self._images_folder, file_name), cv2.IMREAD_COLOR)
        return {
            'img': img,
            'img_show': img_show,
            'file_name': file_name
...
class CocoDemoDataset(Dataset):
    def __init__(self, file_names, images_folder):
        super().__init__()
        with open(file_names, 'r') as f:
            self._file_names = json.load(f)
        self._images_folder = images_folder
