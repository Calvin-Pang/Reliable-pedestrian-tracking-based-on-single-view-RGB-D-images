...
class CropPad:
    def __init__(self, pad, center_perterb_max=40, crop_x=368, crop_y=368,img_input = 3):
        self._pad = pad
    ...
    def __call__(self, sample):
        ...
        cropped_image = np.empty(shape=(self._crop_y, self._crop_x, self._img_input), dtype=np.uint8)
        for i in range(self._img_input):
